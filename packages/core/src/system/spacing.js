// @flow
import { getSpacing } from '../theme.legacy'
...
const isSpaceProp = key => {
  if (key.length === 1) return !!properties[key[0]]
  else if (key.length === 2) return !!properties[key[0]] && !!directions[key[1]]
  return false
}
...
export const space = ({ theme, ...props }: SpaceProps & ThemeProps) => {
  const styles = Object.keys(props).reduce((prev, key) => {
    if (!isSpaceProp(key)) return prev
    return {
      ...prev,
