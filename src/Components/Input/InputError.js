...
class InputErrorComponent extends Component<Props> {
  ...
  componentDidUpdate(oldProps) {
    if (!!this.props.children) this.transitionChildren = this.props.children

    if (this.props.children && !oldProps.children) this.animate(1)
    else if (!this.props.children && oldProps.children) this.animate(0)
  }

  animate(val: number) {
    Animated.spring(this.animation, { toValue: val }).start()
  }
